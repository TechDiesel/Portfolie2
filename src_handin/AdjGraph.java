...
public class AdjGraph { // Defines the AdjancencyGraph class
    ...
    public void newEdge(Vertex from, Vertex to, Integer TEU) { // Adds a new edge between two Vertices, and adds the
                                                               // weights of the note (TEU)
    ...
    public void calcTEUStock() { // Method that calculates the surplus/deficit of the ports
        ...
        for (int i = 0; i < vertices.size(); i++) {
            ...
            currentv.setTEUtotal(-currentv.getTEUsent(), -currentv.getTEUreceived()); // Uses the method
                                                                                      // setTEUtotal(parses the two
                                                                                      // arguments requiered). The total
                                                                                      // TEU is set to sent-received
                                                                                      // TEU.
        }
    }
    ...
    public void printTEU() { // Method that prints the surplus/deficit of the ports
        ...
        for (int i = 0; i < vertices.size(); i++) { // For loop that iterates over list of our list of vertices, and
                                                    // prints the name and total TEU stock to console
            currentv = vertices.get(i);
    ...
    public void reversingFlowCost() { // Method that calculates cost of reversing flow
        ...
        for (int i = 0; i < vertices.size(); i++) { // Doublenestet for loop.
            ...
            for (int j = 0; j < currentv.getOutEdges().size(); j++) {
                ...
                priceOfReversing += currente.getWeight() * pricePerUnit; // cost is updated with the values from method
                                                                         // getWeight, which is the TEU of ports
            }
        }
    ...
    public void cheapestFlow() { // Method to calculate cheapest flow (solving the minimum cashflow problem)
        ...
        while (TEUminus.size() > 0 && TEUplus.size() > 0) { // Whle TEUminus and TEUplus list is greater than 0 (meaning
                                                            // list is not empty)
            // Temp strings that stores the current iteration of the TEUminus and TEUplus
            // names, that we're comparing
            String TEUminusTemp = TEUminus.get(TEUminus.size() - 1).getName();
            String TEUplusTemp = TEUplus.get(TEUplus.size() - 1).getName();

            // Temp ints that stores the current iteration of the TEUminus and TEUplus
            // stock, that we're comparing
            ...
            // Updates TEU values of TEUminus and TEUplus. In other words, transfering
            // containers between the ports
            TEUminus.get(TEUminus.size() - 1)
                    .updateTEUtotal(TEUminus.get(TEUminus.size() - 1).getTEUtotal() + findMinimum);
            ...
            if (TEUminus.get(TEUminus.size() - 1).getTEUtotal() == 0) {
                ...
                System.out.println(
                        "Now removing: " + TEUminusTemp + " from list... Initial stock amount have been satisfied..");
                TEUminus.remove(TEUminus.size() - 1);
                // if current iteration of TEUminus TEU value == 0, then it got correct amount
                // of stock and is removed.

            }
            if (TEUplus.get(TEUplus.size() - 1).getTEUtotal() == 0 && TEUplus.size() > 0) {

                System.out.println(
                        "Now removing: " + TEUplusTemp + " from list... Initial stock amount have been satisfied..");
...
class Vertex {
    ...
    public void updateTEUtotal(Integer teu) { // Setter. Used to update TEUtotal, only after it already has been
                                              // initialized
        TEUtotal = teu;
    }

    @Override
    public String toString() { // toString methods that overrides Java's toString. Used to define how we want
                               // to print our object to the console
...
class Edge {
    ...
    public Edge(Vertex from, Vertex to, Integer cost) { // Constructor
        ...
        fromVertex.addOutEdge(this); // Calls the method addOutEdge with arguments (Vertex from, Vertex to, Integer
                                     // cost). IE. constructs a new edge object
        fromVertex.setTEUsent(weight); // Calls method setTEUsent, with arguments (weight). This sets the sent TEU of
                                       // the edge
        toVertex.setTEUreceived(weight); // calls the method set TEUreceived, with the argument (weight). This sets the
                                         // received TEU of the edge
    }
